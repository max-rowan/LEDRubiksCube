...
RubiksCubeSolver::RubiksCubeSolver(RubiksCube *pCube) : cube(pCube) { cd.init( ); }
...
bool RubiksCubeSolver::_isCross(CubeFace cf, CRGB::HTMLColorCode color) {
  return cf.face[0][1].color == color && cf.face[1][0].color == color &&
         cf.face[1][2].color == color && cf.face[2][1].color == color;
}

bool RubiksCubeSolver::_isSolid(CubeFace cf) {
  for (int y = 0; y < 3; y++) {
    for (int x = 0; x < 3; x++) {
      if (cf.face[y][x].color != cf.color)
        return false;
    }
  }
  return true;
...
void RubiksCubeSolver::_s1BottomEMid( ) {
  ...
  if (cube->pFront->bottomBorderEdge[1]->color != cube->pBottom->color)
    return;

  while (cube->pFront->upperBorderEdge[1]->color == cube->pBottom->color) {
    if (cube->pRight->upperBorderEdge[1]->color != cube->pBottom->color)
      _u( );
    else
      _u_( );
  }
...
void RubiksCubeSolver::_s1BottomMid( ) {
  ...
  if (cube->pFront->face[2][1].color == cube->pBottom->color) {
...
void RubiksCubeSolver::_s1MidRight( ) {
  ...
  if (cube->pFront->face[1][2].color == cube->pBottom->color) {
    // rotate top until there is no bottom color square in rotation path
    while (cube->pRight->upperBorderEdge[1]->color == cube->pBottom->color) {
      if (cube->pBack->upperBorderEdge[1]->color != cube->pBottom->color)
        _u( );
      else
        _u_( );
    }
...
void RubiksCubeSolver::_s1MidLeft( ) {
  ...
  if (cube->pFront->face[1][0].color == cube->pBottom->color) {
    // rotate top until there is no white square in rotation path
    while (cube->pLeft->upperBorderEdge[1]->color == cube->pBottom->color) {
      if (cube->pBack->upperBorderEdge[1]->color != cube->pBottom->color)
        _u_( );
      else
        _u( );
...
bool RubiksCubeSolver::_s2IsEdge( ) {
  return cube->pFront->face[0][1].color == cube->pFront->color &&
         cube->pFront->upperBorderEdge[1]->color == cube->pBottom->color;
}

void RubiksCubeSolver::step2( ) {
  while (!_isCross(*cube->pBottom, cube->pBottom->color)) {
    _s2FindEdge( );
    if (_s2IsEdge( )) {
      _f( );
      _f( );
...
bool RubiksCubeSolver::_validBottomCorners( ) {
  ...
  if (cube->pBottom->face[0][0].color != color || cube->pBottom->face[0][2].color != color ||
      cube->pBottom->face[2][0].color != color || cube->pBottom->face[2][2].color != color)
    return false;

  // Side faces' bottom corners
  if (!cube->pFront->face[2][0].isCorrectColor( ) || !cube->pFront->face[2][2].isCorrectColor( ) ||
      !cube->pRight->face[2][0].isCorrectColor( ) || !cube->pRight->face[2][2].isCorrectColor( ) ||
      !cube->pBack->face[2][0].isCorrectColor( ) || !cube->pBack->face[2][2].isCorrectColor( ) ||
      !cube->pLeft->face[2][0].isCorrectColor( ) || !cube->pLeft->face[2][2].isCorrectColor( ))
    return false;
...
bool RubiksCubeSolver::_isIncorrectBottomCorner( ) {
  return cube->pFront->bottomBorderEdge[0]->color == cube->pBottom->color &&
         cube->pFront->face[2][2].color != cube->pFront->color;
}
...
bool RubiksCubeSolver::_isTopCorner( ) {
  return cube->pFront->face[0][2].color == cube->pBottom->color ||
         cube->pFront->upperBorderEdge[2]->color == cube->pBottom->color ||
         cube->pFront->rightBorderEdge[0]->color == cube->pBottom->color;
}
...
void RubiksCubeSolver::_s3FacingTop( ) {
  ...
  if (cube->pFront->upperBorderEdge[2]->color == cube->pBottom->color) {
    while (cube->pFront->face[0][2].color != cube->pRight->color) {
      _u( );
      cube->turnCube_( );
    }
...
void RubiksCubeSolver::_s3FacingFront( ) {
  ...
  // center tiles
  if (cube->pFront->face[0][2].color == cube->pBottom->color) {
    while (cube->pRight->face[0][0].color != cube->pRight->color) {
      _u( );
      cube->turnCube_( );
    }
    _u( );
...
void RubiksCubeSolver::_s3FacingRight( ) {
  ...
  if (cube->pFront->rightBorderEdge[0]->color == cube->pBottom->color) {
    while (cube->pFront->face[0][2].color != cube->pFront->color) {
      _u( );
      cube->turnCube_( );
    }
    cube->turnCube( );
...
bool RubiksCubeSolver::_s4IsFaceMiddleLayer( ) {
  return cube->pFront->face[1][0].color == cube->pFront->color &&
         cube->pFront->face[1][2].color == cube->pFront->color && !_s4IsWrongMidRight( );
}
...
bool RubiksCubeSolver::_s4IsRotateRight( ) {
  return cube->pFront->face[0][1].color == cube->pFront->color &&
         cube->pFront->upperBorderEdge[1]->color == cube->pRight->color;
}

bool RubiksCubeSolver::_s4IsRotateLeft( ) {
  return cube->pFront->face[0][1].color == cube->pFront->color &&
         cube->pFront->upperBorderEdge[1]->color == cube->pLeft->color;
}

bool RubiksCubeSolver::_s4IsWrongMidRight( ) {
  return cube->pFront->face[1][2].color != cube->pFront->color ||
         cube->pRight->face[1][0].color != cube->pRight->color;
}

bool RubiksCubeSolver::_s4IsWrongMidLeft( ) {
  return cube->pFront->face[1][0].color != cube->pFront->color ||
         cube->pLeft->face[1][2].color != cube->pLeft->color;
}
...
bool RubiksCubeSolver::_s5IsLine( ) {
  return cube->pLeft->upperBorderEdge[1]->color == cube->pUpper->color &&
         cube->pRight->upperBorderEdge[1]->color == cube->pUpper->color;
}

bool RubiksCubeSolver::_s5IsL( ) {
  return cube->pBack->upperBorderEdge[1]->color == cube->pUpper->color &&
         cube->pLeft->upperBorderEdge[1]->color == cube->pUpper->color;
}
...
int RubiksCubeSolver::_s6NumFaceCorners(CubeFace cf) {
  return (cf.face[0][0].color == cf.color) + (cf.face[0][2].color == cf.color) +
         (cf.face[2][0].color == cf.color) + (cf.face[2][2].color == cf.color);
}
...
void RubiksCubeSolver::step6( ) {
  while (!_isSolid(*cube->pUpper)) {
    if (_s6NumFaceCorners(*cube->pUpper) == 0) {
      while (cube->pLeft->face[0][2].color != cube->pUpper->color)
        cube->turnCube( );
      _s6Alg( );
    }

    if (_s6NumFaceCorners(*cube->pUpper) == 1) {
      while (cube->pFront->upperBorderEdge[0]->color != cube->pUpper->color)
        cube->turnCube( );
      _s6Alg( );
    }

    if (_s6NumFaceCorners(*cube->pUpper) == 2) {
      while (cube->pFront->face[0][0].color != cube->pUpper->color)
        cube->turnCube( );
      _s6Alg( );
    }
  }
...
bool RubiksCubeSolver::_s7IsMatchingCorners(CubeFace *cf) {
  return cf->face[0][0].color == cf->face[0][2].color;
}

bool RubiksCubeSolver::_s7IsCornersSolved( ) {
  for (int i = 0; i < 4; i++) {
    if (!_s7IsMatchingCorners(cube->pFront))
      return false;
    cube->turnCube( );
  }
  return true;
...
void RubiksCubeSolver::_s7MatchDoubledPanels( ) {
  while (cube->pFront->face[0][0].color != cube->pFront->color) {
    _u( );
    cube->turnCube_( );
  }
  cube->turnCube_( );
...
bool RubiksCubeSolver::_s8IsSolved( ) {
  return cube->pFront->face[0][1].color == cube->pFront->color &&
         cube->pRight->face[0][1].color == cube->pRight->color &&
         cube->pBack->face[0][1].color == cube->pBack->color &&
         cube->pLeft->face[0][1].color == cube->pLeft->color;
}

bool RubiksCubeSolver::_s8IsBackEdge( ) {
  return cube->pBack->face[0][1].color == cube->pBack->color;
}
...
bool RubiksCubeSolver::_s8IsRight( ) {
  return cube->pFront->face[0][1].color == cube->pRight->color;
}
